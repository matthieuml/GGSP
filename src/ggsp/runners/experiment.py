...
def run_experiment(args: argparse.Namespace):
    ...
    # Load the dataset
    trainset = preprocess_dataset(args.dataset_folder, "train", args.n_max_nodes, args.spectral_emb_dim)
    validset = preprocess_dataset(args.dataset_folder, "valid", args.n_max_nodes, args.spectral_emb_dim)
    testset = preprocess_dataset(args.dataset_folder, "test", args.n_max_nodes, args.spectral_emb_dim)
    ...
    vae_scheduler = torch.optim.lr_scheduler.StepLR(vae_optimizer, step_size=args.vae_scheduler_step_size, gamma=args.vae_scheduler_gamma)
    ...
    denoise_scheduler = torch.optim.lr_scheduler.StepLR(denoise_optimizer, step_size=args.denoise_scheduler_step_size, gamma=args.vae_scheduler_gamma)

    if args.denoise_load_checkpoint_path is not None:
        load_model_checkpoint(denoise_model, denoise_optimizer, args.denoise_load_checkpoint_path)
